...
const mainNavItems: NavItem[] = [
  ...
  { icon: 'airplanemode_active', label: 'UAV Fleet', path: '/fleet' },
  ...
  return (
    <aside className="w-16 md:w-64 bg-white border-r border-neutral-200 flex flex-col shadow-sm">
      <div className="p-4 mb-2 border-b border-gray-100 hidden md:flex items-center">
        ...
        <h1 className="text-lg font-bold bg-gradient-to-r from-primary to-blue-600 bg-clip-text text-transparent">UAV Monitor</h1>
      </div>
      <nav className="flex-1 py-4 overflow-y-auto">
        ...
        <ul className="space-y-1 px-2">
          {mainNavItems.map((item) => (
            <li key={item.path}>
              <Link href={item.path}>
                <a className={`flex items-center px-3 py-2.5 rounded-md transition-colors ${
                  location === item.path 
